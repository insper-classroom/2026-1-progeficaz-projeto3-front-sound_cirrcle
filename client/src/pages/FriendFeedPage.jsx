...
import { getFriendRatings } from "../api/friendsApi";
import StarRating from "../components/StarRating";

function FriendFeedPage() {
  ...
  useEffect(() => {
    getFriendRatings(friendId)
      .then((res) => setRatings(res.data.ratings || []))
      .catch((err) => {
        setError(err.response?.data?.error || "Erro ao carregar avaliações do amigo.");
      })
  ...
  return (
    <div style={{ minHeight: "100vh", background: "radial-gradient(ellipse at center, #071426 0%, #000 60%)" }}>
      ...
      <main style={{ padding: "2rem 1rem", maxWidth: "800px", margin: "0 auto" }}>
        ...
        {error && <div style={{ padding: "0.75rem 1rem", background: "rgba(255,0,0,0.06)", color: "#ff9b9b", borderRadius: "10px", fontSize: "0.85rem" }}>{error}</div>}
        {!loading && ratings.length === 0 && <p style={{ color: "#92a6bd" }}>Este amigo ainda não avaliou nenhuma música.</p>}
